#include <stdio.h>
#include <stdlib.h>
#include <string.h>

typedef struct {
    int id;
    char nome[100];
    int nivel;
} membro;

struct No {
    membro dado;
    struct No* prox;
};

typedef struct {
    struct No* inicio;
} Lista;

void menu() {
    printf("===== Menu de Opcoes =====\n");
    printf("1 - Mostrar Lista\n");
    printf("2 - Criar lista\n");
    printf("3 - Ordenar Lista\n");
    printf("4 - Deletar Lista\n");
    printf("5 - Salvar em Arquivo\n");
    printf("6 - Ler de Arquivo\n");
    printf("7 - Sair\n");
    printf("==========================\n");
}

void ler(membro *pe) {
    printf("Digite o ID, nome e nivel do membro: ");
    scanf("%d %s %d", &pe->id, pe->nome, &pe->nivel);
}

void mostrar(Lista lista) {
    struct No * pi;
    for(pi=lista.inicio; pi!=NULL; pi=pi->prox){ printf("ID: %d, Nome: %s, Nivel: %d\n", pi->dado.id, pi->dado.nome, pi->dado.nivel); }
}

void deletar_lista(Lista *plista) {
    struct No * pi;
    while(plista->inicio != NULL) {
        pi = plista->inicio;
        plista->inicio = pi->prox;
        free(pi);
    }    
}

void inserir_fim(Lista *plista, membro dado) {
    struct No* novo = (struct No*) malloc(sizeof(struct No));
    novo->dado = dado;
    novo->prox = NULL;
    if (plista->inicio == NULL) {
        plista->inicio = novo;
    } else {
        struct No* pi = plista->inicio;
        while (pi->prox != NULL) {
            pi = pi->prox;
        }
        pi->prox = novo;
    }
}

Lista criar_lista(int n) {
    Lista lista;
    lista.inicio = NULL;
    int i;
    for(i=0; i<n; i++) {
        membro e;
        ler(&e);
        inserir_fim(&lista, e);            
    }
    return lista;    
}

void ordenar(Lista *plista) {
    struct No * pi;
    struct No * pj;
    struct No * pfim = NULL;
    if(plista->inicio == NULL) {
        return;
    }
    for(pi=plista->inicio; pi->prox!=NULL; pi=pi->prox) {
        for(pj=plista->inicio; pj->prox!=pfim; pj=pj->prox) {
            if(pj->dado.id > pj->prox->dado.id) {
                membro temp = pj->dado;
                pj->dado = pj->prox->dado;
                pj->prox->dado = temp;
            }
        }
        pfim = pj;
    }
}

void salvar_aquivo(Lista lista) {
    FILE *file = fopen("../guild_roster_098.b", "wb");
    struct No * pi;
    for(pi=lista.inicio; pi!=NULL; pi=pi->prox) {
        fwrite(&pi->dado, sizeof(membro), 1, file);
    }
    fclose(file);
}

Lista ler_arquivo() {
    Lista lista;
    lista.inicio = NULL;
    FILE *file = fopen("../guild_roster_098.b", "rb");
    if(file == NULL) {
        printf("Erro ao abrir o arquivo\n");
        return;
    }
    membro e;
    while(fread(&e, sizeof(membro), 1, file)) {
        inserir_fim(&lista, e);
    }
    fclose(file);
    return lista;
}

int main() {
    Lista lista;
    lista.inicio = NULL;
    int op;
    menu();
    scanf("%d", &op);
    while(op!=7) {
        if(op==1) {
            mostrar(lista);
            if(lista.inicio == NULL) {
                printf("A lista esta vazia\n");
            }
    }
        if(op==2) {
            int n;
            printf("Informe a quantidade de membros:\n");
            scanf("%d", &n);
            lista = criar_lista(n);
        }
        if(op==3) {
            ordenar(&lista);
        }
        if(op==4) {
            deletar_lista(&lista);
            printf("Lista deletada com sucesso\n");
        }
        if(op==5) {
            salvar_aquivo(lista);
            printf("Arquivo salvo com sucesso\n");
        }
        if(op==6) {
            lista = ler_arquivo();
            printf("Arquivo lido com sucesso\n");
        }
        menu();
        scanf("%d", &op);
    }

    return 0;
}
